...
public class UserRequest {
    ...
    @ResponseBody
    private Map<String, Object> getItem(@RequestParam("filters") Optional<String> filter, @RequestParam("id") Optional<String> id){
    ...
    @ResponseBody
    private Map<String,Object> postItem(@RequestBody User body){
    ...
    @ResponseBody
    private Map<String,Object> delItem(@RequestBody User body){
        AtomicReference<JSONObject> response = new AtomicReference<>(new JSONObject());
        Boolean success = true;
        if(body.getId() == null){
            response.get().put("success",false);
    ...
    @ResponseBody
    private Map<String,Object> putItem(@RequestBody User body){
        AtomicReference<JSONObject> response = new AtomicReference<>(new JSONObject());
        Boolean success = true;
        if(body.getId() == null){
            response.get().put("success",false);
    ...
    @ResponseBody
    private Map<String,Object> alterStatus(@RequestBody User body){
        AtomicReference<JSONObject> response = new AtomicReference<>(new JSONObject());
        Boolean success = true;
        if(body.getId() == null){
            response.get().put("success",false);
    ...
    private Map<String,Object> reportAll() throws JRException, FileNotFoundException {
        ...
        report.userReport().ifPresent((item)->{
            response.set(item);
        });
        return response.get().toMap();
    }
